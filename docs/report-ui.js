...
window.StudyReports = (() => {
  ...
  function entry(id,filled=false){
    if(!enabled())return '';
    return `<details class="task-details report-entry" ${buffers.get(id)&&!filled?'open':''}><summary>יש לי דוח מה־AI</summary><p>הדבק כאן את הדוח שקיבלת בסיום המפגש. הוא ימלא טיוטה שאפשר לתקן לפני השמירה.</p><p class="hint">אם עדיין אין דוח: העתק את הבקשה הקצרה לשיחה שבה למדת, והחזר לכאן את התשובה.</p><div class="report-actions"><button class="button small" data-report-action="copy" data-id="${id}">העתקת בקשה לדוח מהמפגש</button><button class="text-link" data-report-action="show" data-id="${id}">הצגת הבקשה</button></div><label class="prompt-label" for="report-paste">הדוח שקיבלת מה־AI</label><textarea class="field-input report-paste" id="report-paste" data-session="${id}" rows="5" spellcheck="false" maxlength="16384" placeholder="מדביקים את בלוק הדוח המובנה שקיבלת מהמורה.">${esc(buffers.get(id)||'')}</textarea><p class="hint">מילוי מהדוח יחליף את שדות התוצאה, העזרה והסיכום בטיוטה. אפשר לבטל את המילוי ולחזור לטיוטה הקודמת.</p><div class="report-actions"><button class="button" data-report-action="apply" data-id="${id}">מילוי הטופס מהדוח</button>${window.STUDY_CONFIG.preview?`<button class="text-link" data-report-action="sample" data-id="${id}">טעינת דוגמה לבדיקה</button>`:''}</div><button class="text-link" data-report-action="clear" data-id="${id}">ניקוי הדוח שהודבק</button><p id="report-error" class="error" role="alert"></p><button class="text-link" data-meeting-action="history" data-id="${id}">היסטוריית המפגשים</button></details>`;
  }
  function showPrompt(id){
    openDialog('report-prompt',id,'בקשה לדוח מהמפגש',`<p>הדבק בשיחה שבה למדת. המורה יסכם את מה שכבר קרה במפגש; אחר כך חזור והדבק כאן את הדוח שלו.</p><textarea class="prompt-preview prompt-textarea" id="prompt-text" rows="12" readonly spellcheck="false">${esc(prompt(id))}</textarea>`,'להדבקה בשיחה עם המורה');
