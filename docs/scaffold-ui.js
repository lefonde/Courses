...
window.StudyScaffold = (() => {
  ...
  async function copy(id,button){
    const text=[StudyLearningEngine.promptContext(state,id),prompt(id,shown(id).id)].filter(Boolean).join('\n\n');
