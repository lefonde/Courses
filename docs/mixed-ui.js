...
window.StudyMixed = (() => {
  ...
  let demoState=null,shownId=null;
  ...
  const remainingBudget=s=>s.minutes-StudyMixedEngine.allocatedMinutes(current(),s.id)-(window.StudyTasks?.recallBudget(s)||0);
  function budgetChoices(){return allSessions().filter(s=>!s.disabled&&isCommitted(s)&&!isDone(s)&&['practice','review'].includes(s.kind)&&arr(s.problemIds).length&&remainingBudget(s)>=10);}
  ...
  async function start(form){
    ...
    await change(draft=>StudyMixedEngine.startRound(draft,{id:'mix-'+crypto.randomUUID(),at:new Date().toISOString(),itemIds,budgetSessionId},content),'הסבב נשמר; אפשר לעצור ולחזור אליו');
  ...
  function showItem(id){
    ...
    shownId=id;const item=itemOf(id),a=attempt(round,id),help=StudyMixedEngine.exposureFor(current(),id),index=round.itemIds.indexOf(id);
    if(!eligible(id)){
  ...
  function makeEvent(id,action,fields={}){return {id:'mix-event-'+crypto.randomUUID(),itemId:id,at:new Date().toISOString(),action,answer:'',outcome:'not-checked',help:StudyMixedEngine.exposureFor(current(),id),note:'',...fields};}
  function draftEvent(round,id,values){
    ...
    const help=strongestHelp(values.help,StudyMixedEngine.exposureFor(current(),id));
    if(!values.answer&&!values.note&&!previous&&help==='none')return null;
    if(previous?.answer===values.answer&&previous?.note===values.note&&strongestHelp(previous?.help,StudyMixedEngine.exposureFor(current(),id))===help)return null;
  ...
  async function reveal(kind){
    ...
    const values=capture(),draft=draftEvent(round,id,values),oldHelp=StudyMixedEngine.exposureFor(current(),id),help=strongestHelp(kind==='solution'?'solution':'hint',strongestHelp(oldHelp,values?.help));
    const alreadyOpened=StudyMixedEngine.historyFor(current(),id).some(e=>e.action===kind);
    const event=alreadyOpened?null:makeEvent(id,kind,{help});
    if(draft||event)await change(d=>{if(draft)StudyMixedEngine.record(d,round.id,draft,content);if(event)StudyMixedEngine.record(d,round.id,event,content);},kind==='hint'?'הרמז תועד והטיוטה נשמרה':'פתיחת הפתרון תועדה');
  ...
  function prompt(){
    ...
    const item=itemOf(id),a=attempt(round,id),values=draftFor(round,id),knownHelp=StudyMixedEngine.exposureFor(current(),id);
    const promptHelp=a?helpLabel[knownHelp]:values.help?'טיוטת דיווח שלי: '+helpLabel[strongestHelp(knownHelp,values.help)]:knownHelp!=='none'?helpLabel[knownHelp]:'טרם דיווחתי על עזרה בשיחה עם המורה; באתר לא תועדה פתיחת עזרה.';
  ...
  async function action(a,id,b){
    ...
    if(a==='hint'||a==='solution')await reveal(a);
